    ...
    if ([keyPath isEqualToString:@"currentPage"]) {
        ...
        for (NSInteger i=0; i<_viewControllers.count; i++) {
            ...
            if (page == i) {
                ...
                if ([className isEqualToString:@"ZYHardlinesChildViewController"]) {
                    ZYHardlinesChildViewController *hardLinesViewController = (ZYHardlinesChildViewController *)viewController;
                    hardLinesViewController.path = _parameters[i];
                }
                if ([className isEqualToString:@"ZYVideosChildViewController"]) {
                    ZYVideosChildViewController *videosViewController = (ZYVideosChildViewController *)viewController;
                     videosViewController.path = _parameters[i];
                }
                [self.strongArray addObject:viewController];

//                if (_parameters && _parameters.count-1 >= i && _parameters[i] && [self getVariableWithClass:viewController.class varName:@"parameter"]) {
//                    [viewController setValue:_parameters[i] forKey:@"parameter"];
//                }
//                if (_parameters && _parameters.count > i) {
//                [RequestBase sendVidoesRequstWithPath:_parameters[i] completionHandler:^(id responseObj, NSError *error) {
//                    
//                }];
//                }
                ...
                CGRect frame = CGRectMake(_selfFrame.size.width * i, 0, _selfFrame.size.width, _scrollView.bounds.size.height);
